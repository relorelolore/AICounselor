...
下列条目写在 §4 中、但 v1 没实现或简化了；v2 再补。本节是「自检表」，
留作未来 PR 的 hook，而不是 bug 列表。

| # | 原 spec 段落 | v1 现状 | 理由 |
|---|---|---|---|
| 1 | §4.3 助手回复中显示 `▍ 检索中…` 工具状态指示 | 未实现；助手只显示通用打字光标 `▍` | 需要 LLM 把 tool_calls 时机显式发到客户端；当前协议只发最终 token / citation / done，无法增量显示检索状态 |
| 2 | §4.4 drawer 在移动端是「底部 sheet」 | 实际仍是右侧 92vw 滑入抽屉 | CSS 工程量与首版价值不匹配（移动端 drawer 用得少）；sheet 需要全新布局 |
| 3 | §4.5 `visualViewport` API 适配键盘弹出 | 未实现；输入框固定贴底 | 移动端键盘遮挡是已知小毛病，但不是阻塞性问题；改输入框定位需要重构 composer 布局 |

第 1 项跟 v2 的增量流式 token 改造（spec §v2.6）一起做才有意义；后两项是 UI 打磨，与核心功能无关。