...
| chromadb 0.6.x × posthog ≥ 7.x 不兼容 | `chromadb/telemetry/product/posthog.py::Posthog._direct_capture` 用 `posthog.capture(distinct_id, event_name, properties)`（3 个位置参数）调 posthog；posthog 7.x 把签名改成 `(event: str, **kwargs)` 只接受 1 个位置参数 → 每个 telemetry 事件失败：`capture() takes 1 positional argument but 3 were given`。错误在签名绑定阶段就抛出，所以光设 `posthog.disabled=True` 或 `ANONYMIZED_TELEMETRY=False` 都无效（chromadb 0.6.x 的 `__settings = Settings()` 单例在 `app/main.py` 跑到 env var 设置之前就已被 import 链构造出来，且 `BaseSettings` 不会在 Settings 实例化后再次重读 env）。修法：在 `app/main.py` 和 `rag/retriever.py` 顶部 monkey-patch `chromadb.telemetry.product.posthog.Posthog._direct_capture`，把它改成用新签名 `posthog.capture(event.name, distinct_id=..., properties=...)`；patch 用 `_AICounselor_patched` 类属性幂等（uvicorn `--reload` / 重复 import 安全），覆盖每条入口：uvicorn / `python -m ingest` / pytest。`scripts/run.sh` / `run.bat` 也加了 `export ANONYMIZED_TELEMETRY=False` 作为对未来 chromadb 版本的防御。 |
...
| 模型 native CoT 不显示 | Qwen / DeepSeek / MiniMax / Anthropic 风格等模型会输出 `<think>` / `<reasoning>` / `<analysis>` / `<scratchpad>` / `<thinking>` / `<reflection>` / `<plan>` / `<\|reasoning\|>` 等思考段；`app/routes_chat.py::_strip_reasoning` 默认 strip。调试：(1) WS 请求加 `show_reasoning: true`（前端 chat URL `?debug=reasoning` 自动启用）；(2) `PUT /api/admin/settings {"debug": {"show_reasoning": true}}` 全局生效（admin settings → 调试 卡片）。两条路径 OR 关系，任一为真即保留 CoT。新模型格式需扩展 `_REASONING_PAIRS`。 |
