...
def test_default_base_url(monkeypatch):
    ...
    assert cfg.get_llm_settings().base_url == "http://localhost:8848/v1"
    assert cfg.get_llm_settings().model_name == "g0chu-Qwen3.6-35B-A3B-NVFP4"
...
def test_env_override(monkeypatch):
    ...
    import importlib, llm.config as cfg
    importlib.reload(cfg)
    assert cfg.get_llm_settings().base_url == "http://h:1/v1"
    assert cfg.get_llm_settings().model_name == "m"
